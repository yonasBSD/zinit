...
#[cfg(target_os = "linux")]
use crate::zinit::ord::{DUMMY_ROOT, ProcessDAG, service_dependency_order};
...
impl LifecycleManager {
    ...
    pub async fn stop<S: AsRef<str>>(&self, name: S) -> Result<()> {
        ...
        let signal = {
            ...
            signal::Signal::try_from(signum).map_err(|_| {
                anyhow::anyhow!("unknown stop signal: {}", service.service.signal.stop)
            })?
        };
    ...
    pub async fn list(&self) -> Result<Vec<String>> {
        let table = self.services.read().await;
        Ok(table.keys().map(std::convert::Into::into).collect())
    }
    ...
    async fn watch_service(self, name: String, input: Arc<RwLock<ZInitService>>) {
        ...
        if service.target == Target::Down {
            debug!("service '{name}' target is down");
            return;
        }

        if service.scheduled {
            debug!("service '{name}' already scheduled");
            return;
        }
        ...
        loop {
            ...
            'checks: loop {
                ...
                debug!("checking {name} if it can schedule");
                if self.can_schedule(&config).await {
                    debug!("service {name} can schedule");
                    break 'checks;
                }
                ...
                // has changed
                debug!("service {name} is blocked, waiting release");
                sig.await;
            }
            ...
            let child = match child {
                ...
                Err(err) => {
                    ...
                    error!("service {name} failed to start: {err}");
            ...
            match result {
                Err(err) => {
                    error!("failed to read service '{name}' status: {err}");
