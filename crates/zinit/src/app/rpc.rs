...
const fn code_name_from(code: i32) -> &'static str {
    match code {
...
fn zinit_err_to_rpc(
    ...
    let (code, code_name, top_msg) = match err.downcast_ref::<ZInitError>() {
        ...
        Some(ZInitError::ServiceAlreadyMonitored { name }) => (
            ...
            format!("service '{name}' already monitored"),
        ),
        Some(ZInitError::ServiceIsUp { name }) => (
            SERVICE_IS_UP,
            "ServiceIsUp",
            format!("service '{name}' is up"),
        ),
        Some(ZInitError::ServiceIsDown { name }) => (
            SERVICE_IS_DOWN,
            "ServiceIsDown",
            format!("service '{name}' is down"),
        ),
...
impl ZinitServiceApiServer for Api {
    ...
    async fn monitor(&self, name: String) -> RpcResult<()> {
        ...
        let (name_str, service) = config::load(format!("{name}.yaml")).map_err(|_e| {
            ErrorObjectOwned::owned(
                CONFIG_ERROR,
                format!("monitoring service: failed to load '{name}.yaml'"),
                Some(json!({
    ...
    async fn create(&self, name: String, content: Map<String, Value>) -> RpcResult<String> {
        ...
        if file_path.exists() {
            return Err(ErrorObjectOwned::owned(
                ...
                format!("creating service: Service '{name}' already exists"),
                Some(json!({
        ...
        let yaml_content = serde_yaml::to_string(&content).map_err(|_e| {
            ErrorObjectOwned::owned(
                CONFIG_ERROR,
        ...
        Ok(format!("Service '{name}' created successfully"))
    }
    ...
    async fn delete(&self, name: String) -> RpcResult<String> {
        ...
        if !file_path.exists() {
            return Err(ErrorObjectOwned::owned(
                SERVICE_NOT_FOUND,
                format!("deleting service: Service '{name}' not found"),
                Some(json!({
        ...
        Ok(format!("Service '{name}' deleted successfully"))
    }
    ...
    async fn get(&self, name: String) -> RpcResult<Value> {
        ...
        if !file_path.exists() {
            return Err(ErrorObjectOwned::owned(
                SERVICE_NOT_FOUND,
                format!("getting service: Service '{name}' not found"),
                Some(json!({
                    "code_name": "ServiceNotFound",
...
impl ZinitSystemApiServer for Api {
    ...
    async fn start_http_server(&self, address: String) -> RpcResult<String> {
        ...
        match Self::start_http_server(self, address).await {
            Ok(result) => Ok(result),
    ...
    async fn stop_http_server(&self) -> RpcResult<()> {
        // Call the method from the API implementation
        match Self::stop_http_server(self).await {
            Ok(()) => Ok(()),
            Err(e) => Err(zinit_err_to_rpc(
                e,
