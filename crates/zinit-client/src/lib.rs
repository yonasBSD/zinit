        ...
        if let RpcError::Call(call) = &err {
            ...
            let human = match (service, action, hint) {
                (Some(s), Some(a), Some(h)) => {
                    format!(
                        "{code_name}[{code}]: {message} while {a} '{s}'. Hint: {h}. Details: {chain:?}"
                    )
                }
                (Some(s), Some(a), None) => {
            ...
            return match code {
                -32000 => Self::ServiceNotFound(human),
                -32001 => Self::ServiceAlreadyMonitored(human),
                -32002 => Self::ServiceIsUp(human),
                -32003 => Self::ServiceIsDown(human),
                -32004 => Self::InvalidSignal(human),
                -32005 => Self::ConfigError(human),
                -32006 => Self::ShuttingDown,
                -32007 => Self::ServiceAlreadyExists(human),
                -32008 => Self::ServiceFileError(human),
                _ => Self::RpcError(human),
            };
        }

        match err {
            RpcError::Transport(_) => Self::ConnectionError(err.to_string()),
...
impl Client {
    ...
    pub async fn unix_socket<P: AsRef<std::path::Path>>(path: P) -> Result<Self, ClientError> {
        Ok(Self::Ipc(path.as_ref().to_string_lossy().to_string()))
    }
    ...
    async fn get_ipc_client(&self) -> Result<impl ClientT, ClientError> {
        match self {
            Self::Ipc(path) => IpcClientBuilder::default()
                .build(path)
                .await
                .map_err(|e| ClientError::ConnectionError(e.to_string())),
